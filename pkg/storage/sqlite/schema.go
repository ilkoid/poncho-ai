...
`
	...
	// StockProductsSchemaSQL defines the stock products metrics table.
	// WB API: POST /api/v2/stocks-report/products/products (Seller Analytics API v2)
	// Grain: one row per (snapshot_date, nm_id).
	// Stores product-level stock metrics: orders, buyouts, sale rate, availability, lost orders.
	const StockProductsSchemaSQL = `
	-- ============================================================================
	-- STOCK PRODUCTS METRICS (Seller Analytics API — /api/v2/stocks-report/products/products)
	-- Grain: one row per (snapshot_date, nm_id)
	-- ============================================================================
	...
	CREATE TABLE IF NOT EXISTS stock_products (
	    ...
	    -- Counts and sums (Swagger: integer uint64 → INTEGER)
	    orders_count INTEGER DEFAULT 0,
	    orders_sum INTEGER DEFAULT 0,
	    avg_orders REAL DEFAULT 0,
	    buyout_count INTEGER DEFAULT 0,
	    buyout_sum INTEGER DEFAULT 0,
	    buyout_percent INTEGER DEFAULT 0,
	    stock_count INTEGER DEFAULT 0,
	    stock_sum INTEGER DEFAULT 0,
	    to_client_count INTEGER DEFAULT 0,
	    from_client_count INTEGER DEFAULT 0,

	    -- Duration metrics (special negatives: -1=infinite, -2=zero, -3=not calc, -4=absent)
	    sale_rate_days INTEGER DEFAULT 0,
	    sale_rate_hours INTEGER DEFAULT 0,
	    avg_stock_turnover_days INTEGER DEFAULT 0,
	    avg_stock_turnover_hours INTEGER DEFAULT 0,
	    office_missing_time_days INTEGER DEFAULT 0,
	    office_missing_time_hours INTEGER DEFAULT 0,

	    -- Lost metrics (Swagger: number float64, special negatives)
	    lost_orders_count REAL DEFAULT 0,
	    lost_orders_sum REAL DEFAULT 0,
	    lost_buyouts_count REAL DEFAULT 0,
	    lost_buyouts_sum REAL DEFAULT 0,

	    -- Avg orders by month (JSON array, rarely queried)
	    avg_orders_by_month TEXT DEFAULT '[]',

	    -- Price range (Swagger: integer uint64 — NOT float)
	    current_price_min INTEGER DEFAULT 0,
	    current_price_max INTEGER DEFAULT 0,
	...
	// GetStockProductsSchemaSQL returns the stock products table schema.
	func GetStockProductsSchemaSQL() string {
		return StockProductsSchemaSQL
	}
