...
// TestSalesReportDetailedPage_FinanceKeyUsedImmediately проверяет, что при
// заданном SetFinanceKey запрос уходит СРАЗУ с этим ключом — без
// предварительной попытки с apiKey. Это критично: каждый лишний запрос под
// тем же seller-UUID насыщает глобальный лимит продавца (429
// "Limited by global limiter, per seller").
func TestSalesReportDetailedPage_FinanceKeyUsedImmediately(t *testing.T) {
	var (
		mu       sync.Mutex
		auths    []string
		reqCount int
	...
	// Ровно один запрос — никаких двойных попыток.
	if got, want := reqCount, 1; got != want {
		t.Errorf("request count = %d, want %d (no double-request)", got, want)
	}
	if len(auths) != 1 {
		t.Fatalf("auths recorded = %d, want 1", len(auths))
	}
	if auths[0] != "WB_API_KEY_value" {
		t.Errorf("Authorization = %q, want WB_API_KEY_value (financeKey first)", auths[0])
	}
	if len(page.Rows) != 1 || page.Rows[0].RrdID != 42 {
		t.Errorf("unexpected page rows: %+v", page.Rows)
	}
}

// TestSalesReportDetailedPage_NoFinanceKeyUsesApiKey проверяет откат к apiKey,
// когда financeKey не задан. Это поведение для вызывающих, которые не знают о
// finance endpoint (никаких регрессий для остальных потребителей *wb.Client).
func TestSalesReportDetailedPage_NoFinanceKeyUsesApiKey(t *testing.T) {
	var (
		mu    sync.Mutex
		auths []string
	)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		mu.Unlock()
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `[{"rrdId":1,"nmId":1}]`)
	}))
	defer srv.Close()
	...
	if len(auths) != 1 || auths[0] != "WB_STAT" {
		t.Errorf("Authorization = %v, want [WB_STAT] (apiKey fallback)", auths)
	}
}
