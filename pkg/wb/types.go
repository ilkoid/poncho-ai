...
// ============================================================================
// NM Report Funnel Types (Seller Analytics API — async CSV reports)
// ============================================================================

// NmReportFunnelRequest creates a task to generate funnel CSV report.
// POST /api/v2/nm-report/downloads
//
// Report types:
//   - DETAIL_HISTORY_REPORT: funnel metrics per nmID per day
//   - GROUPED_HISTORY_REPORT: funnel metrics aggregated by subject/brand/tag per day
type NmReportFunnelRequest struct {
	ID             string               `json:"id"` // UUID generated by seller
	ReportType     string               `json:"reportType"`
	UserReportName string               `json:"userReportName,omitempty"`
	Params         NmReportFunnelParams `json:"params"`
}

// NmReportFunnelParams contains funnel report generation parameters.
// NOTE: Unlike stock-history, funnel reports use flat startDate/endDate (not currentPeriod).
type NmReportFunnelParams struct {
	StartDate        string  `json:"startDate"`                  // YYYY-MM-DD (required)
	EndDate          string  `json:"endDate"`                    // YYYY-MM-DD (required)
	AggregationLevel string  `json:"aggregationLevel,omitempty"` // day (default), week, month
	NmIds            []int64 `json:"nmIDs,omitempty"`            // up to 1000, empty = all
	SubjectIds       []int   `json:"subjectIds,omitempty"`
	BrandNames       []string `json:"brandNames,omitempty"`
	TagIds           []int64 `json:"tagIds,omitempty"`
	SkipDeletedNm    bool    `json:"skipDeletedNm,omitempty"`
