...
// MockCardsSource returns deterministic fake card data.
// Implements CardsSource for --mock mode and testing.
type MockCardsSource struct {
	mu    sync.RWMutex
	cards []wb.ProductCard
}

// NewMockCardsSource creates a mock source pre-populated with cardCount cards.
func NewMockCardsSource(cardCount int) *MockCardsSource {
	m := &MockCardsSource{}
	m.populate(cardCount)
	return m
}

// GetCardsPage returns a page of mock cards.
// Simulates cursor-based pagination per WB Content API spec.
func (m *MockCardsSource) GetCardsPage(_ context.Context, settings wb.CardsSettings) ([]wb.ProductCard, *wb.CardsCursorResponse, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// populate fills mock with deterministic card data.
func (m *MockCardsSource) populate(cardCount int) {
	...
	for i := 0; i < cardCount; i++ {
		...
		cards[i] = wb.ProductCard{
			...
			Wholesale: func() *wb.CardWholesale {
				if i%4 == 0 {
					return &wb.CardWholesale{Enabled: true, Quantum: 10 + i%5}
				}
				return nil
			}(),
...
func makeMockSizes(cardIdx int) []wb.CardSize {
	baseChrtID := 50000 + cardIdx*10
	if cardIdx%3 == 0 {
		return []wb.CardSize{
			{ChrtID: baseChrtID, TechSize: "42", WBSize: "42", Skus: []string{fmt.Sprintf("SKU-%d", baseChrtID)}},
		}
	}
	return []wb.CardSize{
		{ChrtID: baseChrtID, TechSize: "42", WBSize: "42", Skus: []string{fmt.Sprintf("SKU-%d", baseChrtID)}},
		{ChrtID: baseChrtID + 1, TechSize: "44", WBSize: "44", Skus: []string{fmt.Sprintf("SKU-%d", baseChrtID+1)}},
	}
}

func makeMockTags(cardIdx int) []wb.CardTag {
	if cardIdx%4 == 0 {
		return nil
	}
	tags := []wb.CardTag{{ID: 1, Name: "Новинка", Color: "#FF0000"}}
	if cardIdx%3 == 0 {
