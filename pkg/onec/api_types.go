...
// apiGood represents a product from 1C accounting system.
type apiGood struct {
	...
	SKUs []apiSKU `json:"sku"`
}

// apiSKU represents a size variant (barcode) of a product.
type apiSKU struct {
	GUID      string  `json:"guid"`
...
// apiPriceItem represents price data for one product.
...
// apiPrice represents one price type for a product.
...
// apiPIMGood represents a validated product from the PIM system.
type apiPIMGood struct {
	Identifier string                `json:"identifier"`
	Enabled    bool                  `json:"enabled"`
	Family     string                `json:"family"`
	Categories []string              `json:"categories"`
	Values     map[string][]apiValue `json:"values"`
	Created    string                `json:"created"`
	Updated    string                `json:"updated"`
}

// apiValue wraps a single attribute value from PIM.
