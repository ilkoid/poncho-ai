...
// PenaltiesFilterConfig — фильтрация штрафов за габариты (measurement-penalties).
//
// Продуктовый фокус: аналитик фильтрует по конкретным товарам или категориям,
// чтобы приоритетно понять проблему и исправить карточки.
// Все поля опциональны, AND между полями.
type PenaltiesFilterConfig struct {
	// Конкретные nm_id (для фокуса на конкретных товарах)
	NmIds []int `yaml:"nm_ids"` // Например: [12345678, 87654321]. Пусто = все

	// Фильтр по категории товара (case-insensitive contains)
	Subject string `yaml:"subject"` // Например: "Кроссовки". Пусто = все

	// Только подтверждённые (true), только отменённые (false), все (nil)
	IsValid *bool `yaml:"is_valid"` // true = подтверждённые, false = отменённые
}
