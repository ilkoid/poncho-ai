...
- `pkg/onec/` — 1C/PIM v2 downloader domain (1C Goods+Prices API, PIM Goods API). Three-step download: goods+SKUs+dimensions, prices, PIM. Dual-backend (SQLite + PostgreSQL).
...
- `cmd/data-downloaders/` — 24 WB API data collectors (sales, funnel, funnel-agg, promotion, promotion-v2, feedbacks, **feedbacks-v2**, cards, prices, stocks, stock-history, region-sales, **region-sales-v2**, search-visibility, **search-vis-v2**, supplies, **supplies-v2**, 1c-data, **1c-data-v2**, all-articles, orders-v2, opsales-v2, **campaigns-v2**, **funnel-csv-v2**)
...
**PostgreSQL setup — TWO VPS, different defaults (agent: check both):**
...
| onec | ✅ `pkg/onec/` | ✅ | ✅ | ❌ No (full reload). 1C/PIM domain: goods+SKUs+dimensions, prices, PIM. Non-WB API (basic auth, streaming JSON) |
| Others (9) | — | v1 only | — | — |
