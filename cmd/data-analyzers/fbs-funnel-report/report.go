	...
	return []glossarySection{
		{
			...
			Entries: []glossaryEntry{
				...
				{"Лист «Когорты»", "День = день СОЗДАНИЯ заказа. Отвечает на вопрос «что стало с заказами, оформленными в день X?» — единственный честный способ мерить % выкупа: делим на свою же когорту."},
				{"Лист «Динамика»", "День = день ПЕРЕХОДА в текущий статус (updated_at). Отвечает на вопрос «сколько выкупов/отмен случилось в день X?» — суммирует события когорт любых дат."},
				{"Пример", "Заказ оформлен 18.08, выкуплен 26.08 → он в когорте 18.08 (лист «Когорты», колонка «Выкуп»), а факт выкупа — событие 26.08 (лист «Динамика»). Рост выкупов в «Динамике» при плоском % = дозревание больших когорт, а не улучшение конверсии."},
			},
		},
		{
		...
		},
		{
			...
			Entries: []glossaryEntry{
				...
				{"Зрелая когорта", "Возраст когорты ≥ p90 цикла (лист «Скорость ЖЦ», ~12 сут): почти все заказы завершились, проценту можно верить. У незрелых успели завершиться только быстрые → % смещён ВВЕРХ."},
