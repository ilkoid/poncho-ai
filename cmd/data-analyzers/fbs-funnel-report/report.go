	...
	return []glossarySection{
		{
			...
			Entries: []glossaryEntry{
				{"Когорта", "Все заказы, оформленные покупателями в один календарный день (по МСК). «Когорта 18.08» = все FBS-заказы, размещённые 18 августа. Дальше группа «зреет»: часть выкупается, часть отменяется; итог подводят, когда «в пути» ≤ 10% (~2 недели, см. «Зрелая когорта»)."},
		...
		},
		{
			...
			Entries: []glossaryEntry{
				...
				{"Зрелая когорта", "«В пути» ≤ 10% заказов когорты: почти всё завершилось, проценту выкупа можно верить. Возраст ≥ p90 цикла выкупа (~12 сут) — недостаточное условие: p90 считается по выкупам, а отмены «истёк срок хранения» созревают позже."},
				{"Цикл до выкупа (цикл заказ→выкуп)", "Промежуток от создания заказа до выкупа (updated_at − created_at у выкупов). Карточка «Цикл до выкупа» в шапке дашборда — медиана за весь период; медиана и p90 по когортам — в разделе «Скорость ЖЦ»."},
				{"Средний чек выкупа", "Выручка ÷ число выкупов."},
			},
		},
		{
			Title: "ПРИЁМКА НА СЦ (отгрузка)",
			Entries: []glossaryEntry{
				{"Поставка FBS", "Отгрузка собранных FBS-заказов на WB: сборочные задания пакуются в поставку (ID вида WB-GI-…), поставка закрывается передачей в доставку, затем физически принимается на сортировочном центре. Данные — GET /api/v3/supplies (download-wb-fbs-orders-v2, фаза 3). Не путать с поставками товара на склад WB (FBW) из utilities download-wb-supplies."},
				{"Приёмка на СЦ (scan_dt)", "Момент приёмки поставки на сортировочном центре: scan_dt, «дата сканирования поставки или первого заказа» (03-orders-fbs.yaml:4561). В живых данных она на ~0,5–1 ч позже closed_at (передача в доставку) — именно скан означает, что WB физически получил товар."},
				{"Лаг приёмки", "scan_dt − время заказа. На столбиках «Приёмка на СЦ» — медиана по заказам когорты (типичный заказ ждал приёмки N часов); в тултипе — лаг от ПЕРВОГО заказа поставки (медиана/p90/размах по поставкам). Целевой порог — 24 часа от заказа до приёмки. У последних когорт часть поставок ещё не принята — их лаг дорастёт."},
			},
		},
		{
...
func methodologyLines(data *ReportData) []string {
	...
	return []string{
		...
		"• Зрелость когорты = «в пути» ≤ 10% её заказов: у незрелых доля выкупа ещё изменится",
		"  (успели завершиться только быстрые переходы). Колонка «Зрелая когорта».",
