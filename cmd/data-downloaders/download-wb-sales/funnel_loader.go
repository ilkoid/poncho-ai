...
type FunnelLoaderConfig struct {
	...
	Days          int  // Days of history to load (1-365)
	...
	RateLimit     int // Desired rate limit (req/min) — passed to client.Post()
	Burst         int // Burst for rate limiter — passed to client.Post()
}
...
func LoadFunnelHistory(ctx context.Context, cfg FunnelLoaderConfig) (*FunnelLoadResult, error) {
	...
	// Default rate limit (fallback — normally set via SetRateLimit in main.go)
	if cfg.RateLimit <= 0 {
		cfg.RateLimit = 3
	}
	if cfg.Burst <= 0 {
	...
	fmt.Printf("📦 Батчей: %d (по %d товаров)\n", len(batches), cfg.BatchSize)
	fmt.Printf("⏳ Rate limit: %d req/min (адаптивный, через wb.Client)\n", cfg.RateLimit)
	fmt.Println()
	...
	for i, batch := range batches {
		...
		// wb.Client automatically enforces adaptive rate limiting via SetRateLimit()
		productsLoaded, metricsLoaded, err := loadFunnelBatch(ctx, cfg.Client, cfg.Repo, batch, cfg.Days, cfg.RefreshWindow, cfg.RateLimit, cfg.Burst)
		if err != nil {
			fmt.Printf("❌ Ошибка: %v\n", err)
...
func loadFunnelBatch(ctx context.Context, client *wb.Client, repo *sqlite.SQLiteSalesRepository, nmIDs []int, days int, refreshDays int, rateLimit int, burst int) (productsLoaded, metricsLoaded int, err error) {
	// Build request
	now := time.Now()
