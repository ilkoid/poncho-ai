...
**`config.DownloadConfig`** уже содержит: `Days` (от вчерашнего дня), `From`/`To` (точные даты, приоритет над Days), `Resume`, `Rewrite`, `SkipServiceRecords`, `AdaptiveRecoverAfter`, `AdaptiveProbeAfter`, `MaxBackoffSeconds`.
...
- [ ] Флаги: `--config`, `--mock`, `--dry-run`, `--days`, `--begin`, `--end`, `--resume`, `--rewrite`
