...
# Копия run-daily-analytics.sh для VPS #2 (213.208.160.214, ssh -p 16200, user ilkoid).
# Ожидаемый путь клона: /home/ilkoid/go-workspace/src/poncho-ai (= ~/go-workspace/src/poncho-ai).
# Требует прекомпилированные бинарники в bin/ и .env в корне репо.
# Отличия от v1: шапка/документация, префикс лога analytics-v2-*.
# Логика (фазы, git pull, ротация 30 дней, flock) — идентична v1.
# LOCKFILE общий с v1: два экземпляра в одном чекауте не должны писать базу параллельно.

# Корень проекта: каталог самого скрипта (на VPS #2 резолвится в путь выше);
...
mkdir -p "${LOGDIR}"
TODAY=$(date +%Y-%m-%d)
LOGFILE="${LOGDIR}/analytics-v2-${TODAY}.log"
...
# ── Fail fast: prebuilt binaries ──────────────────────────────
if [[ ! -d "${BIN}" ]]; then
    log "ERROR: ${BIN} not found — build first: go build -o bin/ ./cmd/..."
    exit 1
fi
...
log "=== Daily analytics pipeline (v2) started ==="
