...
## Secrets: жёсткий запрет хардкода (token: `secrets_no_hardcode`)

**ХАРДКОД СЕКРЕТОВ ЗАПРЕЩЁН — В КОДЕ, СКРИПТАХ, КОНФИГАХ, АРГУМЕНТАХ КОМАНДНОЙ СТРОКИ.** Никаких паролей, API-ключей и токенов: никогда, не «временно», не «локально», не в комментариях, не в аргументах вызовов. Это правило уровня «MASS WRITES FORBIDDEN» — нарушение требует ротации секрета у владельца.

Единственные допустимые места для секретов:
- `.env` в корне проекта — канонический источник (см. секцию выше), формат `export VAR='...'`;
- окружение машины (`~/.bashrc`, systemd `Environment=`, env демона) — когда `.env` недоступен;
- в коде/скриптах/конфигах — только чтение из env (`"$SMTP_PASSWORD"`, `${PG_PWD}`) или YAML-плейсхолдеры `${VAR}` (как в `cmd/ops-notify/mail-notify/config.yaml`).

Запрещено конкретно:
- передавать секрет аргументом команды (`./run.sh real СЕКРЕТ`) — аргументы видны всем в `ps aux`; env (`/proc/PID/environ`) виден только owner/root — потому env допустим, аргументы нет;
- писать значение в трекаемый/локальный файл «на время»;
- вставлять реальные значения секретов в чаты и AI-сессии (в т.ч. «для проверки»/диагностики).

Инцидент-прецедент (2026-09, `run-readiness-cron.sh:33`): агент захардкодил SMTP-пароль в аргументе вызова; пароль прошёл через три AI-сессии → принудительная ротация у сисадминов. Правка скрипта дыру НЕ закрыла — только ротация. Памятка: **если секрет покинул `.env`/env — он скомпрометирован; сообщить пользователю для ротации немедленно**, удаление из кода/лога после факта remediation-ом не считается. Подозрение на засвет в чате/сессии — проверять `grep -rn` по репо (в git он не должен встречаться вовсе), но logs/сессии не откатываются — только ротация.
